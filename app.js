...
App.Settings = {
    ...
    renderLottiePreview: function(container, animationData, sizePx) {
        ...
        // С сервера animationData приходит СТРОКОЙ (сырой JSON-текст). Раньше она
        // передавалась в player.load() как есть, а если веб-компонент <lottie-player>
        // ещё не успел зарегистрироваться (CDN-скрипт с unpkg подгрузился медленнее,
        // чем отработал app.js), player.load был undefined — и код скатывался в
        // player.setAttribute('src', animationData). Это пихало весь «сырой» JSON
        ...
        // что скрипт с CDN уже успел выполниться к этому моменту.
