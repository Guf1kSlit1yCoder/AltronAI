...
App.Settings = {
    ...
    open: function() {
        ...
        if (user.statusEmoji) {
            document.getElementById('settings-status-preview').innerHTML = `<lottie-player src="${user.statusEmoji}" autoplay loop style="width: 32px; height: 32px;"></lottie-player>`;
        } else {
    ...
    handleStatusSelect: function(e) {
        const file = e.target.files[0];
        if (!file) return;
        const reader = new FileReader();
        reader.onload = (event) => {
            this.pendingStatusData = event.target.result; 
            document.getElementById('settings-status-preview').innerHTML = `<lottie-player src="${this.pendingStatusData}" autoplay loop style="width: 32px; height: 32px;"></lottie-player>`;
        };
        reader.readAsDataURL(file);
    },
    save: async function() {
        ...
        try {
            const payload = { email: user.email, nickname: newNickname || user.nickname };
...
App.Auth = {
    ...
    updateUIProfile: function(userData) {
        ...
        // Рендер Lottie-статуса
        const statusContainers = [document.getElementById('ui-status-container'), document.getElementById('mobile-ui-status-container')];
        statusContainers.forEach(c => {
            if(c) {
                if (userData.statusEmoji) {
                    c.innerHTML = `<lottie-player src="${userData.statusEmoji}" autoplay loop style="width: 20px; height: 20px;"></lottie-player>`;
                } else {
                    c.innerHTML = '';
                }
            }
